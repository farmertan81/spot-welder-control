...
# ---- UART COMMAND INTERFACE (GPIO43/44 -> Pi UART GPIO14/15) ----
...
def cmd_handle(line):
    ...
    try:
        ...
        if up == "GET_STATUS":
            ...
            print_both("STATUS,enabled=%d,state=%s,vpack=%s,i=%s,cooldown_ms=%d,pulse_ms=%d" %
                       (1 if system_enabled else 0, state, vp, ip,
...
try:
    while True:
        ...
        if time.ticks_diff(t_now, _last_print) >= PRINT_MS:
            ...
            print_both("STATUS,enabled=%d,state=%s,vpack=%s,i=%s,cooldown_ms=%d,pulse_ms=%d" %
                       (1 if system_enabled else 0, state, v_str, i_str,
                        max(0, time.ticks_diff(next_weld_ok_at, time.ticks_ms())), PULSE_MS))
            if cells:
                V1, V2, V3, C1, C2, C3 = cells
                print_both("CELLS,V1=%.3f,V2=%.3f,V3=%.3f,C1=%.3f,C2=%.3f,C3=%.3f" %
