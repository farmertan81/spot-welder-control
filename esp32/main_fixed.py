...
def start_tcp_server():
    print("DEBUG: start_tcp_server() called")
    global tcp_server
    print("DEBUG: Creating socket...")
    tcp_server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    tcp_server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    print("DEBUG: Binding to port", TCP_PORT)
...
if ADS1256_AVAILABLE:
    try:
        spi_ads = SPI(1, baudrate=7680000, polarity=0, phase=1,
...
def do_weld_ms(pulse_ms):
    ...
    t_start_us_cached = t_start_us  # Cache this value
    FET_WELD1.on(); FET_WELD2.on()

    # Pre-allocate variables to avoid lookups
    sample_count = 0
    last_v = 8.4  # Default voltage
    
    # Ultra-fast sampling loop - ZERO overhead
    if ADS1256_AVAILABLE and adc_current:
        read_fast = adc_current.read_current_fast  # Cache method reference
        append = weld_samples.append  # Cache append method
        ticks_us = time.ticks_us  # Cache function
        ticks_diff_ms = time.ticks_diff  # Cache function
        ticks_ms = time.ticks_ms  # Cache function
        
        while ticks_diff_ms(ticks_ms(), t_start_ms) < pulse_ms:
            current = read_fast()
            if current is not None:
                append((ticks_us() - t_start_us_cached, last_v, current))
                sample_count += 1
    else:
        # Fallback if no ADS1256
    ...
    # Send all samples to Pi
    for t_us, v, i in weld_samples:
        print_both("WDATA,%.3f,%.1f,%d" % (v, i, t_us))
        time.sleep_ms(1)
...
# ---- WiFi and TCP Server Setup ----
print("DEBUG: About to call setup_wifi() and start_tcp_server()")
try:
    setup_wifi()
    print("DEBUG: setup_wifi() completed successfully")
except Exception as e:
...
try:
    start_tcp_server()
    print("DEBUG: start_tcp_server() completed successfully")
except Exception as e:
    print("TCP server failed:", e)
    import sys
    sys.print_exception(e)
