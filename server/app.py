    ...
    try:
        while True:
            ...
            if message.startswith("HELLO"):
                ...
                # log(f"📡 Broadcasting status: {status_data}")
...
def status_broadcast_thread():
    ...
    while True:
        ...
        if int(time.time() * 10) % 1 == 0:
            with status_lock:
                balance = calculate_cell_balance()
                # log(f"DEBUG esp_status: {esp_status}")
                # log(f"DEBUG cells_status: {cells_status}")
                # log(f"DEBUG temperature: {temperature}")
...
# Initialize ESP32 link
    ...
    try:
        esp_link = ESP32Link(
            host='192.168.68.60',  # UPDATE THIS IP if your ESP32 has a different address
            port=8888,
